...
define(["three", "util", "shaders", "BufferGeometry", "random", "band"],
        (function (THREE, util, shaders, BufferGeometry, Random, Band) {
            ...
            var Scene = function (renderer, width, height) {
                ...
                var scope = this;
                var tid, top, ths, the, bender, lightID;
                var music;
                ...
                this.changeAttr = function (attr) {
                    var explo = scope.scene.getObjectByName("explosion", true);
                    if (explo) {
                        switch (attr) {
                            case 'weight' :
                                explo.material.uniforms.weight.value = $('#explosionWeight').val();
                                break;
                            case 'frequency' :
                                explo.material.uniforms.freqScale.value = $('#frequencyScale').val();
                                break;
                            case 'color' :
                                explo.material.uniforms.colorScale.value = $('#colorScale').val();
                                break;
                            default :
                                console.log("switch changeAttr false");
                        }
                    }
                };
                ...
                this.animate = function (state) {
                    ...
                    } else if (scope.scene.getObjectByName("dLight", true) && state) {
                        // light animation
                        var xRange = range(-1, 1, 0.01).concat(range(0.99, -0.99, 0.01));
                        var zRange = range(1, -1, 0.01).concat(range(-0.99, 0.99, 0.01));
                        var currentXRangeIndex = xRange.indexOf(scope.currentDirectionalLight.position.x);
                        //indexof() gibt immer den ersten index von 2 gleichen werten im range
                        //daher: falls directlight hinter planeten ist muss currentXRangeIndex auf 2. wert zeigen 
                        if (scope.currentDirectionalLight.position.z > 0) {
                            currentXRangeIndex = currentXRangeIndex + (2 * (xRange.length / 2 - currentXRangeIndex));
                        }
                        var currentZRangeIndex = (currentXRangeIndex + xRange.length / 4) % xRange.length;

                        lightID = setInterval(function () {

                            //% soll index out of range verhindern
                            scope.currentDirectionalLight.position.x = xRange[currentXRangeIndex++ % xRange.length];
                            scope.currentDirectionalLight.position.z = zRange[currentZRangeIndex++ % zRange.length];

                            scope.currentMesh.rotation.y -= 0.0020;
                        }, 30);
                    } else {
                        //surface animation
